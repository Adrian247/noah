...
class ReportHtmlBuilder
{
    ...
    public function __construct(
        private readonly ReportCoverRenderer $coverRenderer = new ReportCoverRenderer,
        private readonly ReportPageSettingsNormalizer $pageSettingsNormalizer = new ReportPageSettingsNormalizer,
    ) {}
    ...
    private function renderDocument(
        ...
        $coverHtml = $this->coverRenderer->render(
            $pageSettings['cover_page'] ?? null,
            $company?->name ?? 'Phoenix',
            $asset?->tag ?? 'DEMO-001',
            $routineId,
...
HTML;
    ...
    private function assemblePreviewBody(
        ...
        if ($coverEnabled && $coverHtml !== '') {
            $html .= $this->coverRenderer->themedCoverPageOpeningTag($pageSettings);
...
';
    ...
    {
        return ReportPlaceholders::replace($text, $companyName, $assetTag, $routineId);
    }
    ...
    private function renderParagraphField(
        ...
        $showKey = array_key_exists('show_field_key', $component)
            ? (bool) $component['show_field_key']
            : false;
        $labelCell = $showKey
            ? $this->e($humanLabel).' ('.$this->e($fieldKey).')'
            : $this->e($humanLabel);

        $sectionStyle = $this->sectionStyleClass();
        $tableClass = trim('report-field-table '.$sectionStyle);
        $rowExtra = '';
        if ($sectionStyle === 'report-field-table--card') {
            $rowExtra = 'background:'.$this->themeColor('header_bg', '#f8fafc').';';
        }
        if ($sectionStyle === 'report-field-table--line') {
            $rowExtra = 'border-left:3px solid '.$this->themeColor('accent', '#f59e0b').';';
        }

        return '<table width="100%" cellpadding="5" cellspacing="0" class="'.$tableClass.'" style="margin:0 0 10px 0;font-family:DejaVu Sans,sans-serif;font-size:11pt;border-collapse:collapse;">'
            .'<tr>'
            .'<td width="58%" style="font-weight:bold;vertical-align:top;border-bottom:1px solid '.$this->themeColor('border', '#e5e5e5').';'.$rowExtra.'">'.$labelCell.'</td>'
            .'<td style="vertical-align:top;border-bottom:1px solid '.$this->themeColor('border', '#e5e5e5').';'.$rowExtra.'">'.$this->e($valueText).'</td>'
            .'</tr></table>';
    }

    private function sectionStyleClass(): string
    {
        $style = (string) ($this->documentTheme['section_style'] ?? 'minimal');

        return match ($style) {
            'card' => 'report-field-table--card',
            'line' => 'report-field-table--line',
            default => 'report-field-table--minimal',
        };
    }
    ...
    {
        return ReportPublicImageEmbedder::imgTag($path, $class);
    }
